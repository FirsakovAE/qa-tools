...
function isExpectedExtensionError(e) {
...
let devtoolsSearchPort = null;
chrome.runtime.onConnect.addListener((port) => {
    if (port.name !== SEARCH_PORT_NAME)
        return;
...
async function syncSettingsToChromeStorage(settings) {
    try {
...
async function sendMessageWithRetry(tabId, message, messageType, maxRetries = 3) {
            ...
            try {
                chrome.runtime.sendMessage(message).catch((e) => {
                    if (!isExpectedExtensionError(e))
                        console.error('[background] Pinia relay sendMessage failed:', e);
                });
            }
            catch (e) {
                if (!isExpectedExtensionError(e))
                    console.error('[background] Pinia relay sendMessage threw:', e);
            }
            break;
