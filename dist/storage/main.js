 ...
 */
import { STORAGE_PREFIX, STORAGE_RESPONSE_PREFIX, DB_NAME, DB_VERSION, SETTINGS_STORE, MEDIA_STORE, MEDIA_LIMIT_BYTES, } from './storage-protocol';
// ────────────────────────────────────────
 ...
 */
async function requestUnpartitionedStorage() {
...
async function setSettings(key, data) {
    const database = await openDB();
    return new Promise((resolve, reject) => {
...
async function setMedia(id, blob) {
    ...
    if (newTotal > MEDIA_LIMIT_BYTES) {
        throw new Error(`Media limit exceeded: ${(newTotal / 1024 / 1024).toFixed(1)} MB / ${MEDIA_LIMIT_BYTES / 1024 / 1024} MB`);
    }
    const database = await openDB();
    return new Promise((resolve, reject) => {
...
async function migrateFromLegacyDatabases() {
    ...
    if (oldSettingsDb) {
        try {
            if (oldSettingsDb.objectStoreNames.contains('kv')) {
                ...
                if (settings)
                    await setSettings('inspector-settings', settings);
            }
        }
        catch (e) {
        ...
        if (oldMediaDb.objectStoreNames.contains('blobs')) {
            const entries = await new Promise((resolve, reject) => {
                ...
                req.onsuccess = () => {
                    ...
                    if (cursor) {
                        if (cursor.value instanceof Blob) {
                            list.push({ key: cursor.key, blob: cursor.value });
                        }
                        cursor.continue();
                    }
                    else {
...
window.addEventListener('message', async (event) => {
    ...
    if (!data || typeof data !== 'object' || !data[STORAGE_PREFIX])
        return;
