 ...
 */
export function buildCurlCommand(entry) {
    const parts = ['curl'];
    ...
    for (const header of entry.requestHeaders) {
        ...
        if (header.name.toLowerCase() === 'content-length')
            continue;
        if (isFormData && header.name.toLowerCase() === 'content-type')
            continue;
    ...
    if (isFormData) {
        ...
        const tryPrettify = entry.requestBody.text.trim().length > 0 &&
...
function parseUrlForPostman(rawUrl) {
    try {
...
function entryToPostmanItem(entry) {
    const isFormData = entry.requestBody?.formData && entry.requestBody.formData.length > 0;
...
export function buildPostmanCollection(entries, name) {
    return {
        info: {
...
export function downloadPostmanCollection(entries, name) {
    try {
