...
function createContext(visited) {
    return {
 ...
 */
function safeGetKeys(value, ctx) {
    if (shouldAbort(ctx))
        return [];
    try {
...
// ============================================================================
function unwrapRef(value, maxDepth = MAX_REF_UNWRAP_DEPTH) {
    let current = value;
    let depth = 0;
...
// ============================================================================
function serializeMap(map, ctx, depth) {
    if (!trackNode(ctx))
        return { __type: 'Map', __timeout: true };
    const result = { __type: 'Map', entries: [] };
    let count = 0;
    try {
...
function serializeSet(set, ctx, depth) {
    if (!trackNode(ctx))
        return { __type: 'Set', __timeout: true };
    const result = { __type: 'Set', values: [] };
    let count = 0;
    try {
...
function serializeArray(arr, ctx, depth) {
    if (!trackNode(ctx))
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (typeof item === 'function' || isDOMNode(item))
            continue;
...
// ============================================================================
function serializeObject(value, ctx, depth) {
    if (!trackNode(ctx))
    ...
    for (const key of keys) {
        ...
        if (typeof propValue === 'function' || isDOMNode(propValue))
            continue;
...
// ============================================================================
function serializeValue(value, ctx, depth) {
 ...
 */
export function serializeProps(props, visited) {
 ...
 */
export function serializeRawPropsForDeclared(rawProps) {
    ...
    try {
        for (const key of Object.keys(rawProps)) {
            ...
            else if (v && typeof v === 'object' && typeof v.nodeType === 'number') {
    ...
    return serializeProps(normalized);
}
/**
 * Lightweight check if a value can be safely serialized
 */
export function canSerialize(value) {
    if (value === null || value === undefined)
        return true;
    const type = typeof value;
 ...
 */
export function getSerializationStats(props) {
