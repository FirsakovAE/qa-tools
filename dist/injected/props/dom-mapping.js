 ...
 */
export function findComponentRootEl(el) {
    if (!el)
        return null;
    // 1. Если сам элемент — визуальный корень UI-компонента
    if (el.classList?.contains('or-select') ||
    ...
    while (current && current !== document.body) {
        if (current.classList?.contains('or-select') ||
...
export function generateUniqueSelector(element) {
    ...
    while (current && current !== document.body) {
        ...
        if (current.className && typeof current.className === 'string') {
            const classes = current.className.trim().split(/\s+/).filter(cls => /^[a-zA-Z][a-zA-Z0-9_-]*$/.test(cls) && // Только валидные CSS классы
        ...
        const similarSiblings = siblings.filter(sibling => {
            if (sibling.tagName !== current.tagName)
                return false;
            const currentClasses = (current.className || '').split(/\s+/).filter(cls => !cls.includes('animate-') && !cls.includes('transition-') && !cls.includes('backdrop-'));
            const siblingClasses = (sibling.className || '').split(/\s+/).filter(cls => !cls.includes('animate-') && !cls.includes('transition-') && !cls.includes('backdrop-'));
...
//# sourceMappingURL=dom-mapping.js.map