 ...
 */
export function extractRawProps(instance) {
    if (!instance)
        return null;
    // Vue 3
    if (instance.props) {
 ...
 */
export function getLivePropsForEditing(instance) {
    return extractRawProps(instance);
}
/**
 * Get Vue instance from ComponentMeta
 */
function getVueInstance(meta) {
 ...
 */
function createPropsHash(props) {
 ...
 */
export function readComponentProps(meta) {
 ...
 */
export function readPropsByUid(uid) {
    const store = getMetaStore();
 ...
 */
export function readPropsWithRawByUid(uid) {
    const store = getMetaStore();
    const meta = store.getByUid(uid);
    if (!meta)
        return null;
 ...
 */
export function readPropsWithRawByMeta(meta) {
    const instance = getVueInstance(meta);
    const raw = extractRawProps(instance);
    if (!raw || typeof raw !== 'object')
        return { props: {}, rawProps: {} };
 ...
 */
export function readPropsByMeta(meta) {
    return readComponentPropsForce(meta);
}
/**
 ...
 */
function readComponentPropsForce(meta) {
    const instance = getVueInstance(meta);
    const rawProps = extractRawProps(instance);
    if (!rawProps) {
        return {
 ...
 */
export function readExpandedComponentsProps() {
    const store = getMetaStore();
 ...
 */
export function expandAndReadProps(uid) {
    const store = getMetaStore();
    const meta = store.getByUid(uid);
    if (!meta)
        return null;
 ...
 */
export function collapseAndClearProps(uid) {
    const store = getMetaStore();
    const meta = store.getByUid(uid);
    if (!meta)
        return;
 ...
 */
export function batchReadPropsExplicit(uids) {
 ...
 */
export function hasPropsChanged(uid) {
    const store = getMetaStore();
    const meta = store.getByUid(uid);
    if (!meta)
        return false;
 ...
 */
export function enablePropsLogging(uid) {
 ...
 */
export function disablePropsLogging(uid) {
 ...
 */
export function getPropsHistory(uid) {
 ...
 */
export function updateLoggedComponentsProps() {
    if (!loggingEnabled)
        return [];
    const store = getMetaStore();
    const diffs = [];
    for (const uid of loggedComponents) {
        ...
        if (prevHash !== newHash) {
            // Props changed
            const prevProps = meta.propsSnapshot?.raw;
            // Update snapshot
            meta.propsSnapshot = {
                raw: rawProps,
                lastUpdated: Date.now(),
...
//# sourceMappingURL=props-reader.js.map