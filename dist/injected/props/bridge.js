...
import { readPropsWithRawByUid, readPropsWithRawByMeta, expandAndReadProps, collapseAndClearProps, readExpandedComponentsProps, extractRawProps } from './props-reader';
...
function getPropsCountsLight(instance) {
    const raw = extractRawProps(instance);
    if (!raw || typeof raw !== 'object')
        return { passed: 0, declared: 0 };
 ...
 */
function metaToLegacyFormatLight(meta) {
...
function isBlacklisted(name, blacklist) {
    if (!blacklist?.active?.length)
        return false;
    const n = name || '';
    if (blacklist.inactive?.some((rule) => likeMatch(n, rule)))
        return false;
    return blacklist.active.some((rule) => likeMatch(n, rule));
}
/** Lightweight format - no serialized props, safe for 64MB limit */
function getLegacyComponents(forceRefresh = false, blacklist, rootElementUid) {
 ...
 */
function findMetaByStableId(stableId) {
 ...
 */
function getComponentProps(componentPath, componentPathFallback) {
