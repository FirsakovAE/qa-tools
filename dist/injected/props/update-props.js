 ...
 */
function preserveType(oldValue, newValue) {
 ...
 */
function unwrapAndAssign(oldValue, newValue) {
 ...
 */
function isModelProp(instance, key) {
    const propName = key.replace(/^-/, '');
    const updateEvent = `update:${propName}`;
    // Vue 3
    if (instance?.emitsOptions?.[updateEvent])
        return true;
    // Vue 2 fallback
    if (instance?.$listeners?.[updateEvent])
        return true;
    return false;
}
/**
 ...
 */
function updateModelProp(instance, key, value) {
    const propName = key.replace(/^-/, '');
    const updateEvent = `update:${propName}`;
    if (instance?.emit) {
 ...
 */
function getPropsFromInstance(instance) {
    if (!instance)
        return null;
    // Vue 3
    if (instance.props)
 ...
 */
export function updateComponentProps(path, newProps) {
    let instance = null;
    let props = null;
 ...
 */
function findComponentByUidFallback(uid) {
    try {
        ...
        function findInTree(vnode, path = '') {
            if (!vnode)
                return null;
...
//# sourceMappingURL=update-props.js.map