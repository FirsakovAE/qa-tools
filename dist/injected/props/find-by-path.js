 ...
 */
const vnodeByElementCache = new WeakMap();
...
let pathCacheCleanupTimer = null;
/**
 * Start periodic cleanup of pathCache
 */
function startPathCacheCleanup() {
 ...
 */
function cleanupPathCacheInternal() {
    const now = Date.now();
    const toDelete = [];
 ...
 */
function setInPathCache(key, vnode) {
    if (pathCache.size >= PATH_CACHE_MAX_SIZE) {
        cleanupPathCacheInternal();
    }
    pathCache.set(key, {
 ...
 */
export function findVueComponentByElement(element) {
    // Check cache first
 ...
 */
function findVNodeByElement(vnode, element, visited) {
    if (!vnode || visited.has(vnode))
        return null;
    visited.add(vnode);
    // Check current vnode
 ...
 * @returns Found VNode component or null
 */
export function findComponentByPath(componentUid) {
    // Check cache first
 ...
 */
function navigateByPath(vueRoots, actualPath, expectedName, expectedUid) {
 ...
 * Navigate to subTree
 */
function navigateSubTree(current, isVue2) {
    if (current.component?.subTree) {
        return current.component.subTree;
    }
    if (isVue2) {
        if (current.child?.component?.subTree)
            return current.child.component.subTree;
        if (current.child)
            return current.child;
        if (current.componentInstance?.$vnode)
            return current.componentInstance.$vnode;
    }
    return null;
}
/**
 ...
 */
function navigateChildren(current, part, isVue2) {
 ...
 */
function navigateVNodeChildren(current, part) {
 ...
 */
function getVNodeName(vnode, isVue2) {
 ...
 */
export function resolveComponent(componentUid) {
 ...
 */
function resolveActualVNode(componentVNode) {
    const vueContext = detectVueContext();
    const isVue2 = vueContext.version === 2;
    let instance = null;
 ...
 */
function resolveByNameFallback(componentUid) {
    const parts = componentUid.split('::');
    const expectedName = parts[1];
    if (!expectedName)
        return null;
 ...
 */
function findComponentByRealVNode(expectedName) {
    const vueRoots = findVueRoots();
    for (const root of vueRoots) {
        const rootVNode = extractRootVNode(root);
        if (!rootVNode)
 ...
 */
function findInTree(vnode, expectedName, visited) {
    if (!vnode || visited.has(vnode))
        return null;
    visited.add(vnode);
    // Check current vnode
 ...
 */
export function disposePathCache() {
    pathCache.clear();
    stopPathCacheCleanup();
}
//# sourceMappingURL=find-by-path.js.map