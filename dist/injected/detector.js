 ...
 */
function setupHookListeners(hook) {
    if (hookInstalled)
        return;
    hookInstalled = true;
    // Store original emit
 ...
 */
function interceptAppUse(app) {
    if (!app || !app.use || app.__piniaIntercepted)
        return;
 ...
 */
export function detectPinia() {
    if (piniaDetected)
        return true;
    // Via window._s
    try {
        const windowS = window._s;
 ...
 */
export function setupReactiveDetection(cbs) {
    callbacks = cbs;
