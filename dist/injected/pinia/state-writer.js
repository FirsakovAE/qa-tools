 ...
 */
export function writePrimitive(target, key, value) {
 ...
 */
function mergeLikePiniaInPlace(target, patch) {
...
function assignRefValueFromJson(ref, newValue) {
    if (!isVueRef(ref) || isComputedRef(ref))
        return;
 ...
 */
function assignToPiniaPatchRoot(patchRoot, key, newValue) {
 ...
 */
export function patchState(storeId, path, value) {
 ...
 */
export function replaceState(storeId, newState) {
    const store = getStore(storeId);
    if (!store) {
        return false;
    }
 ...
 */
export function patchGetters(storeId, newGetters) {
    const store = getStore(storeId);
    if (!store) {
        return {
            success: false,
    ...
    const updated = []; // Keys where real state was updated
    const keyFailures = [];
    const stateKeysGlobal = new Set(Object.keys(store.$state || {}));
    const applyOneKey = (key, newValue, patchRoot) => {
