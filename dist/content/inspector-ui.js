 ...
 */
import { featureFlags, injectedScriptLoaded, setInjectedScriptLoaded, resetDetectionState, uiBridgeInitialized } from './state';
...
export function injectInspectorUI() {
  ...
  `;
    ...
    for (const dir of ['n', 's', 'e', 'w', 'ne', 'nw', 'se', 'sw']) {
        ...
        switch (dir) {
            case 'n':
                h.style.cssText = `${b}top:0;left:${C}px;right:${C}px;height:6px;cursor:ns-resize;`;
                break;
            case 's':
                h.style.cssText = `${b}bottom:0;left:${C}px;right:${C}px;height:6px;cursor:ns-resize;`;
                break;
            case 'e':
                h.style.cssText = `${b}top:${C}px;right:0;bottom:${C}px;width:6px;cursor:ew-resize;`;
                break;
            case 'w':
                h.style.cssText = `${b}top:${C}px;left:0;bottom:${C}px;width:6px;cursor:ew-resize;`;
                break;
            case 'ne':
                h.style.cssText = `${b}top:0;right:0;width:${C}px;height:${C}px;cursor:ne-resize;`;
                break;
            case 'nw':
                h.style.cssText = `${b}top:0;left:0;width:${C}px;height:${C}px;cursor:nw-resize;`;
                break;
            case 'se':
                h.style.cssText = `${b}bottom:0;right:0;width:${C}px;height:${C}px;cursor:se-resize;`;
                break;
            case 'sw':
                h.style.cssText = `${b}bottom:0;left:0;width:${C}px;height:${C}px;cursor:sw-resize;`;
                break;
        }
        h.addEventListener('pointerdown', (e) => {
            ...
            const onMove = (ev) => {
                ...
                if (dir.includes('e'))
                    nw = Math.max(mw, sfw + dx);
                if (dir.includes('w'))
                    nw = Math.max(mw, sfw - dx);
                if (dir.includes('s'))
                    nh = Math.max(mh, sfh + dy);
                if (dir.includes('n')) {
                    nh = Math.max(mh, sfh - dy);
                    npy = sfy + (sfh - nh);
                }
    ...
    function floatingPanelLeft() {
        return Math.round(floatingX - (floatingWidth - 72) / 2);
    }
    function floatingPanelTop() {
    ...
    function detectSnapZone(px, py) {
        ...
        if (py >= vh - SNAP_THRESHOLD)
            return 'bottom';
        if (py <= SNAP_THRESHOLD)
            return 'top';
        if (px <= SNAP_THRESHOLD)
            return 'left';
        if (px >= vw - SNAP_THRESHOLD)
            return 'right';
        return null;
    }
    ...
    // ═══════════════════════════════════════════════════════════
    let collapsedBeforePropsInspect = null;
    propsInspectOverlayHooks = {
        ...
        onSessionEnd() {
            if (collapsedBeforePropsInspect === null)
                return;
