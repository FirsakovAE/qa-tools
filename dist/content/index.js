 ...
 */
import { setDetectionStopped, setDetectionCompleted, setFeatureFlags, uiInjected, setUiInjected, staticSiteDetection, setStaticSiteDetection, checkTimeout, setCheckTimeout } from './state';
import { removeMessageListener } from './detection';
import { cleanupHighlight, setupHighlightEventListeners, removeHighlightEventListeners } from './highlight';
...
// === GLOBAL GUARD ===
if (!window.__VUE_INSPECTOR_CONTENT_LOADED__) {
    ;
    window.__VUE_INSPECTOR_CONTENT_LOADED__ = true;
    // === URL GUARD ===
            ...
            try {
                if (chrome?.runtime?.id) {
                    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
                        if (message?.type !== 'FORCE_INJECT_UI')
                            return false;
                        if (!uiInjected) {
                            setUiInjected(true);
                            injectInspectorUI();
