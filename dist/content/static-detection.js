...
export function detectStaticSite() {
    ...
    const hasVueIndicators = window.__VUE__ ||
        ...
        Array.from(document.querySelectorAll('script')).some(s => s.src?.includes('vue') || s.textContent?.includes('Vue.'));
    if (hasVueIndicators) {
    ...
    const mpaLinks = links.filter(a => {
        ...
        return href && (href.match(/\.html$/) || // .html files
    ...
    const hasRouterIndicators = window.history?.state?._isRouter || // Vue Router state
    ...
    const hasStaticGeneratorIndicators = !!document.querySelector('meta[name="generator"][content*="Hugo"]') ||
