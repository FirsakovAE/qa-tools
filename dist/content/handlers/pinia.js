 ...
 */
export const handlePiniaGetStoresSummary = (message, sender, sendResponse) => {
    requestWindow({ type: 'PINIA_GET_STORES_SUMMARY' }, 'PINIA_STORES_SUMMARY_DATA', 2000)
        ...
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_GET_STORES_SUMMARY failed:', error);
        const result = {
            type: 'PINIA_STORES_SUMMARY_DATA',
            summary: {},
            detected: false,
 ...
 */
export const handlePiniaGetStoreState = (message, sender, sendResponse) => {
    requestWindow({
        ...
        .catch((error) => {
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_GET_STORE_STATE failed:', error);
        const result = {
            type: 'PINIA_STORE_STATE_DATA',
            storeId: message.storeId,
            state: null,
 ...
 */
export const handlePiniaSearch = (message, sender, sendResponse) => {
 ...
 */
export const handlePiniaPatchState = (message, sender, sendResponse) => {
    requestWindow({
        ...
        .catch((error) => {
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_PATCH_STATE failed:', error);
 ...
 */
export const handlePiniaReplaceState = (message, sender, sendResponse) => {
    requestWindow({
        ...
        .catch((error) => {
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_REPLACE_STATE failed:', error);
 ...
 */
export const handlePiniaPatchGetters = (message, sender, sendResponse) => {
    requestWindow({
        ...
        .catch((error) => {
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_PATCH_GETTERS failed:', error);
 ...
 */
export const handlePiniaCallAction = (message, sender, sendResponse) => {
    requestWindow({
        ...
        .catch((error) => {
        if (error?.message !== 'Timeout')
            console.error('[content/handlers/pinia] PINIA_CALL_ACTION failed:', error);
