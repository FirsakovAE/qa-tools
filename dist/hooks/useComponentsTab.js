...
import { PropsEditorServiceFactory } from '@/services/propsEditorService';
import { useInspectorSettings } from '@/settings/useInspectorSettings';
import { likeMatch } from '@/utils/likeMatch';
import { useRuntime } from '@/runtime';
import { isInFavorites } from '@/utils/favoritesMatcher';
export function useComponentsTab(treeData, options = {}) {
    ...
    Promise.all([
        runtime.storage.get(STORAGE_KEY_SEARCH),
        runtime.storage.get(STORAGE_KEY_PROPS_ONLY)
    ...
    const updateEditedProp = (key, value) => {
        editedProps.value[key] = value;
    };
    const saveEditedProps = async () => {
        if (!editingComponent.value) {
            return false;
        }
        try {
            const success = await propsEditorService.value.updateComponentProps(editingComponent.value, editedProps.value);
            if (success) {
    ...
    const debounceDelay = computed(() => settingsRef.value?.searchParams?.debounce ?? 300);
    const minLength = computed(() => settingsRef.value?.searchParams?.minLength ?? 2);
    // Создаем ref для текущей debounced функции
    const debouncedApply = ref(null);
    ...
    const isBlocked = (node) => {
        ...
        const isInInactive = blacklist.inactive.some((rule) => likeMatch(node.name, rule));
        if (isInInactive)
            return false;
        // Проверяем, попадает ли компонент под паттерны из active
        const isInActive = blacklist.active.some((rule) => likeMatch(node.name, rule));
        return isInActive;
    };
    // Функция проверки, находится ли элемент в избранном (используем stable matching)
    const isFavorite = (node) => {
        if (!settingsRef.value?.favorites)
            return false;
            ...
            return nodes
                ...
                if (q && searchSettings) {
                    ...
                    if (!searchCheck && searchSettings.byValue === true && node.props) {
                        ...
                        const searchInValue = (value, searchQuery) => {
                    ...
                    searchCheck = Boolean(node.name.toLowerCase().includes(q) ||
    ...
    const elementsCount = computed(() => {
        const walk = (nodes) => nodes.reduce((sum, n) => sum + 1 + (n.children ? walk(n.children) : 0), 0);
...
//# sourceMappingURL=useComponentsTab.js.map