...
import { fetchLatestRelease, compareVersions, } from '@/services/githubReleaseService';
...
export function useUpdateChecker() {
    ...
    const shouldCheckForUpdates = async () => {
        try {
            const lastCheck = await runtime.storage.get(LAST_CHECK_KEY);
            if (lastCheck) {
    ...
    const isVersionIgnored = async (version) => {
        try {
            const ignoredVersion = await runtime.storage.get(IGNORED_VERSION_KEY);
    ...
    const navigateToAbout = (release) => {
        window.dispatchEvent(new CustomEvent('vue-inspector:navigate-about', {
            detail: { release },
        }));
    };
    const showUpdateToast = (remoteVersion, release) => {
        try {
