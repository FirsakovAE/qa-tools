 ...
 */
export function buildUrlPreview(scheme, host, port, path, query) {
    let url = `${scheme || '*'}://${host || '*'}`;
    if (port)
        url += `:${port}`;
    url += path || '/*';
 ...
 */
export function getStatusClass(status, pending = false) {
    if (pending)
 ...
 */
export function formatBodyForDisplay(text, contentType) {
    if (!text?.trim())
        return '';
    ...
    const shouldTryJson = declaresJson || ((contentType == null || contentType === '') && looksLikeJsonValue(text));
    if (shouldTryJson) {
        try {
            const parsed = JSON.parse(text);
            if (typeof parsed === 'string') {
 ...
 */
export function detectLanguage(contentType) {
    if (!contentType)
        return 'json';
    const ct = contentType.toLowerCase();
    if (ct.includes('json'))
