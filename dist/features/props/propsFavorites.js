 ...
 */
export function getPropsFavoriteNodeId(node) {
 ...
 */
export function isPropsRowResolvedFavorite(node, favorites, allRows) {
    if (!favorites.length)
        return false;
    const getId = getPropsFavoriteNodeId;
    for (const fav of favorites) {
        if (!matchFavoriteIds(getId(node), fav.id))
            continue;
        ...
        if (marked.id === node.id)
            return true;
    }
    return false;
}
//# sourceMappingURL=propsFavorites.js.map