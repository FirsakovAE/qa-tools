...
chrome.runtime.sendMessage({ type: 'GET_DISPLAY_MODE' }, (modeResp) => {
    ...
    if (modeResp?.displayMode !== 'devtools')
        return;
    chrome.runtime.sendMessage({ type: 'IS_STATIC_SITE', tabId }, (staticResp) => {
        ...
        if (staticResp?.isStatic)
            return;
        chrome.devtools.panels.create('Vue Inspector', 'icons/icon32.png', `injected_ui/index.html?devtools=1&tabId=${tabId}`, (panel) => {
