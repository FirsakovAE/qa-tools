...
import { safeSendMessage } from '@/utils/extensionBridge';
...
const state = reactive(structuredClone(defaultInspectorSettings));
let isLoaded = false;
    ...
    (async () => {
        try {
            ...
            if (isStandaloneMode()) {
                ...
                const wpIds = new Set(wallpapers.map(w => w.id));
...
function migrateJsonMode(saved) {
...
function migrateFavoriteIds(saved) {
    ...
    if (changed) {
        const seen = new Set();
        saved.favorites = saved.favorites.filter((fav) => {
            if (seen.has(fav.id))
                return false;
            seen.add(fav.id);
            return true;
...
function migrateSearchSettings(saved) {
    ...
    if (!saved.networkTableColumns || typeof saved.networkTableColumns !== 'object') {
        ...
        if (saved.networkTableColumns.name === undefined)
            saved.networkTableColumns.name = false;
...
function migrateCustomizeSettings(settings) {
    ...
    if (img && img.sourceType === 'file' && img.savedFileId?.startsWith('wallpaper_') && img.fileName && (!img.wallpapers || img.wallpapers.length === 0)) {
        ...
        const videoMimes = { mp4: 'video/mp4', webm: 'video/webm', ogv: 'video/ogg', mov: 'video/quicktime' };
        const mime = videoMimes[ext] || 'image/png';
        img.wallpapers = [{ id: img.savedFileId, name: img.fileName, size: 0, mimeType: mime }];
        if (!img.wallpaperId)
            img.wallpaperId = img.savedFileId;
    }
 ...
 */
export function useInspectorSettingsSync() {
...
// Основная функция
export async function useInspectorSettings() {
    if (!isLoaded) {
        ...
        for (const sf of data.savedFiles || []) {
            if (!sf.dataUri) {
                try {
                    ...
                    if (blob)
                        sf.dataUri = await blobToDataUri(blob);
                }
                catch (e) {
...
//# sourceMappingURL=useInspectorSettings.js.map