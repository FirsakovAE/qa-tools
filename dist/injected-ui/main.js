...
function initRuntime() {
    try {
        ...
        if (standaloneMatch) {
            ...
            try {
                isStandalone = !!window.parent?.__VUE_INSPECTOR_STANDALONE__;
                baseURL = window.parent?.__VUE_INSPECTOR_BASE_URL__ || '';
            }
            catch (e) {
                if (!isExpectedCrossOriginError(e))
                    console.error('[injected-ui/main] Cross-origin parent access failed:', e);
            }
        }
        if (isStandalone) {
