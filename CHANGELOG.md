...
* Изменена логика сбора данных с учетом blacklist: игнорируемые элементы исключаются на этапе предварительного обхода, а не только скрываются из таблицы.
* Это уменьшает влияние heavy-компонентов и ускоряет загрузку вкладки.
...
* Добавлено избранное для **Pinia Store**.
* Элементы Store можно сохранять в избранное.
* В разделе **Options** доступно создание и редактирование пользовательских избранных элементов.
...
* **Standalone** версия переведена на собственное локальное хранилище.
* Настройки сохраняются между запусками и доступны на разных сайтах без потери данных.
...
* На широких экранах используется двухпанельный режим.
...
* Ускорена загрузка вкладки **Props** при наличии компонентов с большим количеством props.
* Исключены избыточные вычисления для blacklist-элементов.
* Снижено влияние heavy nodes на общий лимит props.
